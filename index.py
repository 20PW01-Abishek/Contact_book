                                                                     ...
                                                                     #CONTACT BOOK MANAGEMENT USING GUI#
                                                                       #PYTHON PROGRAMMING LAB PACKAGE#
...
root = Tk()
root.title("Contact book management")
width = 1000
height = 500
...
def UpdateData():
    if GENDER.get() == "":
       result = tkMessageBox.showwarning('', 'Please enter the details in all the fields!', icon="warning")
    else:
        tree.delete(*tree.get_children())
        conn = sqlite3.connect("contact.db")
        cursor = conn.cursor()
